...
**D. 动态 Dock 图标切换（macOS 专属，Windows 下隐藏）**
...
- Windows 下此 UI 通过 `detectIsWindows()` 完全隐藏
...
#### 2.3.3 使用场景 A：Dock 图标切换（仅 macOS）

> **Windows 用户看不到此功能。** 设置 → 外观中，"应用图标"选择器在 Windows 下完全隐藏（`detectIsWindows()` 条件渲染），因为 Windows 没有 Dock 概念。

**功能入口**：设置 → 外观 → "应用图标"选择器（仅 macOS 可见）
...
4. **两个目录必须全部替换，文件名不变**
5. 在 macOS 上进入设置 → 外观 → 应用图标选择器，确认预览图已更新
6. 进入设置 → 机器人平台 → 品牌素材，下载一张确认是新图

> Windows 用户：设置 → 外观 中没有应用图标选择器，但品牌素材下载页仍然可见可用，替换后需在该页面确认预览图。
...
- [ ] macOS 启动台 / Finder 图标
- [ ] 窗口标题栏图标
- [ ] （仅 macOS）设置 → 外观 → 应用图标选择器各变体预览正常
- [ ] （仅 macOS）切换 Dock 图标变体后生效
