...
└── mcp.json                       # ← 已有：保留兼容，逐步迁移到 connectors/
```
...
**Skill 扫描逻辑**：
1. SDK 扫描 `skills/` 目录
2. 同时扫描 `connectors/*/skill/` 目录（仅当该连接器 `enabled: true`）
3. **不复制/链接**：直接让 SDK 扫描连接器目录下的 skill，保持文件结构清晰

**实现方式示例**：
```ts
function getConnectorSkillDirs(workspaceSlug: string) {
  const connectorsConfig = readConnectorsConfig(workspaceSlug)
  const skillDirs = []

  for (const [name, config] of Object.entries(connectorsConfig.connectors)) {
    if (!config.enabled) continue
    if (config.type !== 'cli') continue

    const skillDir = join(getConnectorsDir(workspaceSlug), name, config.skillDir || 'skill')
...
**效果**：
- 飞书 CLI 连接器启用 → SDK 可以扫描到 `feishu-cli/skill/` 中的 Skill
...
| `agent-workspace-manager.ts` | 修改 | 新增 `readConnectorsConfig()`、预置连接器同步逻辑 |
| `agent-orchestrator.ts` | 修改 | `buildMcpServers()` 新增从 `connectors/` 目录加载 MCP 的逻辑 |
| `ipc.ts` | 修改 | 新增连接器相关的 IPC 接口 |
| `electron-builder.yml` | 修改 | 添加 `default-connectors/` 到 `extraResources` |
| `default-connectors/` | 新增 | 预置连接器模板目录 |
| 前端 AgentSettings | 修改 | UI 重构，支持连接器管理 |
...
| 状态总控文件 | ✅ `connectors.json` | 统一管理所有连接器的启停状态 |
