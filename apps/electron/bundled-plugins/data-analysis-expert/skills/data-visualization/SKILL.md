...
description: "用于创建有效的数据可视化。优先使用 plotly 生成交互式图表，seaborn 和 matplotlib 作为静态图表备选；适用于图表类型选择、发布级图表和可访问性设计。"
---
...
fig.update_yaxes(matches=None)  # Independent y-axes if needed
fig.show()
```
