...
3. 若识别到飞书文档 URL，直接执行 `python ${CLAUDE_PLUGIN_ROOT}/skills/po-skills/run.py doc-convert --url "<飞书URL>"`；不要询问是否改用 `lark-doc`，不要要求用户二次确认。脚本会自动转调 `lark-doc-to-md`；如无 REQID / `--output-dir`，默认输出到 `raw/<飞书标题>/`，标题获取失败时回退到 `raw/<飞书文档token>/`。
...
7. 完成后如有图片引用，按图片数量决定是否继续图片转换 / `enhance-content`：图片数量 ≤ 20 时继续加载 `steps/enhance-content.md` 并执行；图片数量 > 20 时，先询问用户是否需要转换图片，用户确认后再执行 `enhance-content`。
