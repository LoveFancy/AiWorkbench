...
7. 完成后如有图片引用，按图片数量决定是否继续图片转换 / `enhance-content`：图片数量 ≤ 20 时继续加载 `${CLAUDE_PLUGIN_ROOT}/skills/po-skills/steps/enhance-content.md` 并执行；图片数量 > 20 时，先询问用户是否需要转换图片，用户确认后再执行 `enhance-content`。
