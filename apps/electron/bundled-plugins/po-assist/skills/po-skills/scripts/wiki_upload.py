...
def build_md2conf_command(
    ...
    *,
    ...
    domain, path = _normalize_confluence_base_url(base_url)
    command = [md2conf_bin, str(source_path), "-d", domain, "-p", path, "-a", token, "--api-version", api_version]
    if username:
        command += ["-u", username]
    if space_key:
        command += ["-s", space_key]
...
def publish_markdown_to_confluence(
    ...
    *,
    ...
    resolved_page_id = page_id or _extract_page_id(command_output) or _extract_page_id(sync_source.read_text(encoding="utf-8"))
    result = WikiUploadResult(
...
def main() -> None:
    ...
    parser.add_argument("--parent-page-id", default="", help="父页面 ID")
    ...
    if opts.mode == "create" and not opts.space_key.strip():
    ...
    try:
        result = publish_markdown_to_confluence(
            ...
            token=token,
            space_key=opts.space_key.strip() or None,
            root_page_id=opts.parent_page_id.strip() or None,
