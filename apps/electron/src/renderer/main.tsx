...
function AgentSettingsInitializer(): null {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const validAgentChannelIds = getAgentAvailableChannelIds(channels)
      ...
      const currentAgentModel = settings.agentChannelId && settings.agentModelId
        ...
        : null
      const resolvedAgentModel = resolveAgentSelectedModel(channels, currentAgentModel)
      setAgentChannelId(resolvedAgentModel?.channelId ?? null)
      setAgentModelId(resolvedAgentModel?.modelId ?? null)
      if (
        resolvedAgentModel?.channelId !== currentAgentModel?.channelId ||
        resolvedAgentModel?.modelId !== currentAgentModel?.modelId ||
        settings.agentChannelIds?.join('\0') !== validAgentChannelIds.join('\0')
      ) {
