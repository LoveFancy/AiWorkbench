...
function PluginMarketContent({ query, onInstalled }: { query: string; onInstalled: () => void | Promise<void> }): React.ReactElement {
  ...
  const handleAddMarketplace = async (): Promise<void> => {
    if (!sourceInput.trim() || adding) return
    setAdding(true)
    try {
    ...
    <div className="flex flex-col gap-4">
      ...
      <Dialog open={addOpen} onOpenChange={(open) => { if (!adding) setAddOpen(open) }}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" disabled={adding} onClick={() => setAddOpen(false)}>取消</Button>
            <Button disabled={!sourceInput.trim() || adding} onClick={() => void handleAddMarketplace()}>
