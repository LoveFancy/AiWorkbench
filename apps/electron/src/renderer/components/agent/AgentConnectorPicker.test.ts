...
describe('AgentConnectorPicker helpers', () => {
  test('只展示已配置或可用的连接器', () => {
    expect(getAvailableConnectorsForPicker(config, connectorsConfig, false).map((item) => item.displayName)).toEqual(['华泰邮箱', '飞书 CLI', 'HiAgent 泰为', 'docs'])
    // 华泰邮箱因为有 email entry 所以 isConfigured=true；docs 也是自定义连接器，isConfigured=true
    expect(getAvailableConnectorsForPicker(config, connectorsConfig, false).filter((item) => item.isConfigured).map((item) => item.name)).toEqual(['email', 'docs'])
  })
