...
async function findBashInPath(): Promise<string | null> {
  try {
    ...
    const paths = decodeCommandOutput(stdout).trim().split(/\r?\n/)
    ...
    // where bash 失败通常表示 PATH 中无 bash
    return null
  }
}
...
export async function detectGitBash(): Promise<GitBashStatus> {
  ...
  for (const path of getCommonGitBashPaths()) {
    ...
    if (version) {
      console.log(`[Git Bash 检测] 找到 Git Bash: ${path} (v${version})`)
      return {
        available: true,
        path,
        version,
  ...
  if (gitInstallPath) {
    ...
    for (const path of candidatePaths) {
      const version = await verifyBashPath(path)
      if (version) {
        console.log(`[Git Bash 检测] 找到 Git Bash（注册表）: ${path} (v${version})`)
        return {
          available: true,
          path,
          version,
  ...
  if (pathBash) {
    ...
    if (version) {
      console.log(`[Git Bash 检测] 找到 Git Bash（PATH）: ${pathBash} (v${version})`)
      return {
        available: true,
        path: pathBash,
        version,
