...
export function buildAgentReplyCard(result: FormattedAgentResult, subtitle?: string): Record<string, unknown> {
  ...
  return {
    ...
    header: {
      title: { tag: 'plain_text', content: 'WorkMate Agent' },
...
export function buildNotificationCard(
  ...
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'WorkMate 任务完成' },
      template: 'green',
    },
    elements: [
      {
      ...
      ...(toolLine ? [
        ...
        {
          tag: 'note',
          elements: [{ tag: 'plain_text', content: `${toolLine} | 在 WorkMate 中查看完整回复` }],
        },
      ] : []),
    ],
  }
...
export function buildErrorCard(errorMessage: string): Record<string, unknown> {
  return {
    config: { wide_screen_mode: true },
    header: {
      title: { tag: 'plain_text', content: 'WorkMate 错误' },
      template: 'red',
    },
    elements: [
