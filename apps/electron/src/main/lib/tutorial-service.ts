...
export function createWelcomeConversation(): ConversationMeta | null {
  ...
  try {
    // 1. 创建对话
    const meta = createConversation('了解 HtAiDevAssist')

    // 2. 保存教程文件为附件
    const attachmentId = randomUUID()
    const attachmentFilename = 'HtAiDevAssist 使用教程.md'
    ...
    const userMessage: ChatMessage = {
      id: randomUUID(),
      role: 'user',
      content: '请帮我了解 HtAiDevAssist 的功能和使用方式，我附上了完整的使用教程作为参考。',
      createdAt: now,
    ...
    const assistantMessage: ChatMessage = {
      id: randomUUID(),
      role: 'assistant',
      content: `你好，欢迎使用 HtAiDevAssist！我已经阅读了完整的使用教程。你可以向我提问来快速了解 HtAiDevAssist，比如：

- HtAiDevAssist 可以做什么？
- 如何配置 AI 供应商渠道？
- Chat 和 Agent 模式有什么区别？
- 什么是 Skills 和 MCP？
- 如何远程使用 HtAiDevAssist？
