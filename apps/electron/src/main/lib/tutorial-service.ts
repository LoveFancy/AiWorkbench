...
export function createWelcomeConversation(): ConversationMeta | null {
  ...
  try {
    // 1. 创建对话
    const meta = createConversation('了解 HtAiWorkBench')

    // 2. 保存教程文件为附件
    const attachmentId = randomUUID()
    const attachmentFilename = 'HtAiWorkBench 使用教程.md'
    ...
    const userMessage: ChatMessage = {
      id: randomUUID(),
      role: 'user',
      content: '你好，我是 HtAiWorkBench 的新用户，希望快速上手。这是完整的使用教程，作为你的参考。',
      createdAt: now,
    ...
    const assistantMessage: ChatMessage = {
      id: randomUUID(),
      role: 'assistant',
      content: `你好，欢迎来到 HtAiWorkBench！HtAiWorkBench 是一个通用的 Agent，其实它可以完成任何事，说实话这也挺难的，因为你要构建完整的工作环境才能做到，这会涉及到一些新的概念或者思考方式，不过别担心，我们做了很多设计可以帮助你靠谱稳定的越用越好用。
