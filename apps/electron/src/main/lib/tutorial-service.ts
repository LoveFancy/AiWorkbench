...
export function createWelcomeConversation(): ConversationMeta | null {
  ...
  try {
    // 1. 创建对话
    const meta = createConversation('了解 WorkMate 伴行')
    ...
    const userMessage: ChatMessage = {
      id: randomUUID(),
      role: 'user',
      content: '你好，我是 WorkMate 伴行的新用户，希望快速上手。这是完整的使用教程，作为你的参考。',
      createdAt: now,
...
了解你的背景之后，我会为你单独整理一份专属的 WorkMate 伴行使用最佳实践——告诉你哪些功能最值得用、推荐的 Skills / MCP 配置，以及贴合你场景的工作流模板。

你也可以先访问安装和使用教程：<https://linkapp.htsc.com.cn/S/019dT1>
