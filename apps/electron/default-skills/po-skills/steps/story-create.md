...
- **已包含** → 进入阶段 D
- **不包含** → 自动执行 story-analyze（加载 `<技能根目录>/steps/story-analyze.md`），完成后进入阶段 D
...
```csv
story_key,story名称,story描述,所属完整迭代名,所属需求编号,所属需求名称,发布版本,经办人工号,经办人姓名,创建人工号,计划开发完成日期,计划测试完成日期,计划完成日期,story_id
...
- `发布版本`：可选。填写完整发布版本名称时，创建 Story 前会按名称查询 DPMP 并写入发布版本；留空则不向 DPMP 传发布版本字段，不再使用默认版本
...
表中迭代、经办人、日期等字段需人工补充；发布版本可选，留空则不传。是否继续创建 DPMP Story？
```
...
不要在执行前手工读取配置文件。直接调用脚本创建 Story；如果脚本返回缺少 `DPMP_COOKIE` 或其他 DPMP 配置，再提示用户补充后重试。
...
```bash
python3 <技能根目录>/run.py story-create \
